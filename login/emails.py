...
def send_credentials(password,name,receiver):
    ...
    text_content = render_to_string('email/passwordemail.txt', {"password":password, "username":name})
    html_content = render_to_string('email/passwordemail.html', {"password":password, "username":name})
